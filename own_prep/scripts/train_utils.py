...
def valid_test_epoch_v2(name, dataset, model, device, data_loader, criterion, limit=None):
    model.eval()
...
def validate(model, checkpoint_path, dataset, valid_loader, device, args):
    ...
    valid_test_epoch_v2(name='Valid',model=model, dataset = dataset, device=device,data_loader=valid_loader,criterion=criterion)